...
#include "VertGameSession.h"
#include "VertOnlineGameSettings.h"
...
AVertSession::AVertSession(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		OnCreateSessionCompleteDelegate = FOnCreateSessionCompleteDelegate::CreateUObject(this, &AVertSession::OnCreateSessionComplete);
		OnDestroySessionCompleteDelegate = FOnDestroySessionCompleteDelegate::CreateUObject(this, &AVertSession::OnDestroySessionComplete);

		OnFindSessionsCompleteDelegate = FOnFindSessionsCompleteDelegate::CreateUObject(this, &AVertSession::OnFindSessionsComplete);
		OnJoinSessionCompleteDelegate = FOnJoinSessionCompleteDelegate::CreateUObject(this, &AVertSession::OnJoinSessionComplete);

		OnStartSessionCompleteDelegate = FOnStartSessionCompleteDelegate::CreateUObject(this, &AVertSession::OnStartOnlineGameComplete);
	}
}
...
*/
void AVertSession::OnStartOnlineGameComplete(FName InSessionName, bool bWasSuccessful)
{
...
void AVertSession::HandleMatchHasStarted()
{
...
*
*/
void AVertSession::HandleMatchHasEnded()
{
	// start online game locally and wait for completion
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
...
bool AVertSession::IsBusy() const
{
...
EOnlineAsyncTaskState::Type AVertSession::GetSearchResultStatus(int32& SearchResultIdx, int32& NumSearchResults)
{
	SearchResultIdx = 0;
	NumSearchResults = 0;
...
*/
const TArray<FOnlineSessionSearchResult> & AVertSession::GetSearchResults() const
{
...
* @param bWasSuccessful true if the async action completed without error, false if there was an error
*/
void AVertSession::OnCreateSessionComplete(FName InSessionName, bool bWasSuccessful)
{
...
* @param bWasSuccessful true if the async action completed without error, false if there was an error
*/
void AVertSession::OnDestroySessionComplete(FName InSessionName, bool bWasSuccessful)
{
...
bool AVertSession::HostSession(TSharedPtr<const FUniqueNetId> UserId, FName InSessionName, const FString& GameType, const FString& MapName, bool bIsLAN, bool bIsPresence, int32 MaxNumPlayers)
{
	...
	if (OnlineSub)
	{
		...
		{
			HostSettings = MakeShareable(new FVertOnlineSessionSettings(bIsLAN, bIsPresence, MaxPlayers));
...
void AVertSession::OnFindSessionsComplete(bool bWasSuccessful)
{
...
void AVertSession::ResetBestSessionVars()
{
	CurrentSessionParams.BestSessionIdx = -1;
}

void AVertSession::ChooseBestSession()
{
...
void AVertSession::StartMatchmaking()
{
	ResetBestSessionVars();
	ContinueMatchmaking();
}

void AVertSession::ContinueMatchmaking()
{
	ChooseBestSession();
...
void AVertSession::OnNoMatchesAvailable()
{
	UE_LOG(LogOnlineGame, Verbose, TEXT("Matchmaking complete, no sessions available."));
	SearchSettings = NULL;
}

void AVertSession::FindSessions(TSharedPtr<const FUniqueNetId> UserId, FName InSessionName, bool bIsLAN, bool bIsPresence)
{
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
	{
		...
		if (Sessions.IsValid() && CurrentSessionParams.UserId.IsValid())
		{
			SearchSettings = MakeShareable(new FVertOnlineSearchSettings(bIsLAN, bIsPresence));
...
bool AVertSession::JoinSession(TSharedPtr<const FUniqueNetId> UserId, FName InSessionName, int32 SessionIndexInSearchResults)
{
	bool bResult = false;
...
bool AVertSession::JoinSession(TSharedPtr<const FUniqueNetId> UserId, FName InSessionName, const FOnlineSessionSearchResult& SearchResult)
{
	bool bResult = false;
...
* @param bWasSuccessful true if the async action completed without error, false if there was an error
*/
void AVertSession::OnJoinSessionComplete(FName InSessionName, EOnJoinSessionCompleteResult::Type Result)
{
...
bool AVertSession::TravelToSession(int32 ControllerId, FName InSessionName)
{
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
	{
...
void AVertSession::RegisterServer()
{
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
	{
		...
		{
			TSharedPtr<class FVertOnlineSessionSettings> ShooterHostSettings = MakeShareable(new FVertOnlineSessionSettings(false, false, 16));
